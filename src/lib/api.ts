...
export interface MenuItem {
  ...
  new_tab: boolean;
  order: number;
  published?: boolean; // Optional to match backend
  parent_id?: string; // Optional to match backend
  children: MenuItem[];
  created_at?: string; // Optional from backend
  updated_at?: string; // Optional from backend
}
...
// Enhanced generic fetch function with better error handling
async function apiFetch<T>(
  endpoint: string,
  options: RequestInit = {}
): Promise<T> {
